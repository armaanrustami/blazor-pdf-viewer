...
using Custom.BlazorPdf.Config;
using Custom.BlazorPdf.Extensions;
...
public partial class PdfViewer : ComponentBase
{
    ...
    [Parameter] public bool EnablePinchZoom { get; set; } = true;
    ...
    [Parameter] public bool EnableRotation { get; set; } = true;
    [Parameter] public bool EnablePrint { get; set; } = true;
    [Parameter] public bool EnableDownload { get; set; } = true;
    [Parameter] public bool EnableDrawing { get; set; } = true;
    [Parameter] public bool EnableMetadata { get; set; } = true;
    [Parameter] public bool EnableThumbnails { get; set; } = true;
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        if (firstRender && PdfFile is not null)
            await PdfInterop.InitializeAsync(ObjectReference!, PdfFile, Config.UseProjectWorker, EnablePinchZoom);
    ...
    [JSInvokable]
    public async Task PinchZoomChanged(double scale)
    {
        if (!EnablePinchZoom || PdfFile is null)
            return;

        if (PdfFile.Zooming.SetScale(scale))
        {
