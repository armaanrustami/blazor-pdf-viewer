...
import {enablePinchZoom} from "./PinchZoom";
...
export async function initPdfViewer(dotnetReference: DotNetObject, pdfDto: PdfState, useProjectWorker: boolean, pinchZoom: boolean = true): Promise<void> {
    ...
    try {
        ...
        if (pinchZoom) enablePinchZoom(pdf.id, dotnetReference);
